...
Even if some are already installed in your main Python installation, you should **reinstall packages needed for your project in the local `venv`** (after activating it). This is considered best practice, as it ensures **isolation** across projects and **reproducibility**. After (re-)installing them, you can export a **`requirements.txt`** file to document the exact versions of all installed packages (this is particularly useful for sharing your environment, e.g., via *GitHub*):
</div>
```{bash, eval=F}
