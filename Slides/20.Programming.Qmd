...
Hypothetical **automated decision** in a **pre-registered analysis pipeline**:
...
<div class="large-code">
**`np.where()`** *(manages one single condition, similar to `ifelse()` in R)*
```{python}
agesVector = np.array([2, 28, 15, 11, 4, 67, 0, 42, 14, 8])
...
<div class="large-code">
**`np.select()`** *(multiple nested conditions, like `dplyr::case_when()` in R)*
```{python}
...
## Loops in Python - Monte Carlo Simulation 😃
...
import numpy as np
np.random.seed(111) # set seed for reproducibility: best practice! 
N = 50
niter = 10
effsize= 0.3
pvalues = np.empty(niter) # initialize empty vector: best practice!

for i in range(niter):
    x1 = np.random.normal(0, 1, size=N)
    x2 = np.random.normal(effsize, 1, size=N)
    tt = ttest_ind(x1,x2)
    pvalues[i] = tt.pvalue.round(3)
...
niter = 10000
effsize= 0.3
pvalues = np.empty(niter)

for i in range(niter):
    x1 = np.random.normal(0, 1, size=N)
    x2 = np.random.normal(effsize, 1,
                                size=N)
    tt = ttest_ind(x1,x2)
    pvalues[i] = tt.pvalue
...
Iterating using an integer index in a range (“`i in range(niter)`” is a common practice, but you can also iterate directly over the elements of a *List* or other data structures (even *Strings*)
...
<div style="font-size:24px;"><em>but note that the latter could be obtained much more easily with `numpy` vectorized operations: <b>`np.array(numbers) ** np.array(exponents)`</b></em></div>
...
<div class="large-code">
```{python}
x = ["apple", "banana", [1,2,3,4], "watermelon"]
result = map(len, x) 
...
**`lambda`** command allows you to *define a custom function in a single line of code* without `def` or `return`; it may be useful for quick transformation, but of course does not allow any complex "logic" / statement
...
**`lambda`** can take more than one argument 
...
rescale = lambda x, m, s: ((x - x.mean()) / x.std()) * s + m

rescale(myVector, 100, 15)
```
</div>
