...
-   First of all, we choose a model from <a href="https://huggingface.co/" target="_blank">HuggingFace</a> (we must specify the exact `"Model Name"`);
-   Then, we load and initialize both the `model` itself and its `tokenizer` (i.e., the function that splits text into tokens \[subword units with meaning, that the language models can understand\]);
-   Finally, we initialize the pipeline for the `"sentiment-analysis"` *task* using that model and tokenizer
...
```{python, cache=T}
sentence = tokenizer("My day was so-so, but afterall not so bad", return_tensors="pt")
print(sentence)
```
...
```{python, cache=T}
logitScores = model(**sentence).logits
print(logitScores)
```
...
-   To summarize "valence" as one single score, we could multiply *negative* -1 times, *neutral* 0 times, *positive* +1 times, so the final valence will be in a continuous range of \[-1, 1\]:
